...
//
package org.rebecalang.rmc.corerebeca.ltl.gov.nasa.ltl.trans;
//Written by Dimitra Giannakopoulou, 19 Jan 2001

import org.rebecalang.rmc.corerebeca.ltl.gov.nasa.ltl.graph.*;
...
class Automaton {
  ...
  public static Graph SMoutput (State[] automaton) {
    ...
    int                            size = Pool.assign();
    org.rebecalang.rmc.corerebeca.ltl.gov.nasa.ltl.graph.Node[] nodes = new org.rebecalang.rmc.corerebeca.ltl.gov.nasa.ltl.graph.Node[size];

    for (int i = 0; i < size; i++) {
      if ((automaton[i] != null) && 
              (i == automaton[i].get_representativeId())) {
        nodes[i] = new org.rebecalang.rmc.corerebeca.ltl.gov.nasa.ltl.graph.Node(g);
